...
| `SchedulerInterface`                     | Separates _what work was scheduled_ from _whether the queue ran it_. The unit tests assert the schedule; the integration tests assert Action Scheduler executes it.                                                    |
| `SettlementModeProviderInterface`        | Supplies a typed `SettlementMode`, allowing `BrowserOnly`, `Hybrid` and `WorkerOnly` to be selected at the composition root without conditionals spreading through settlement.                                         |
| `PaymentPageSettlementObserverInterface` | Gives the gateway one observation operation while the composition root selects either guarded live verification or the structurally cache-only implementation.                                                         |
...
The settlement-mode seam remains typed across that filter. The default graph
constructs `FixedSettlementModeProvider(SettlementMode::Hybrid)`, and a caller
replaces the provider rather than passing a free-form mode string. The enum is
string-backed only for boundaries that must serialize it.
